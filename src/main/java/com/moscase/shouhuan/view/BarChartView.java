...
public class BarChartView extends View {
    ...
    private float mTxtSize = sp2px(8);
    ...
    public BarChartView(Context context, AttributeSet attrs) {
        ...
        mXAxisTxtColor = mTypedArray.getColor(R.styleable.BarChartView_xAxisTxtColor,
                mXAxisTxtColor);
        mYAxisTxtColor = mTypedArray.getColor(R.styleable.BarChartView_yAxisTxtColor,
                mYAxisTxtColor);
...
//        顶部text画笔
        ...
        topTextPaint.setColor(mXAxisTxtColor);
        ...
        topTextPaint.setTextSize(sp2px(7));
    ...
    private void drawRect(Canvas canvas) {
        ...
        for (int i = 0; i < progress.length; i++) {
            ...
            Rect rectOvew = new Rect();
            rectOvew.left = startX + range * i + padding;
            rectOvew.top = caculate(mYMaxValue) - 20;
            rectOvew.right = startX + range * i + range;
            rectOvew.bottom = getHeight() - dp2px(28);
            ...
            if (progress[i] != 0) {
                if (progress[i] > mYMaxValue + 500) {
                    canvas.drawRect(rectOvew, rectPaint);
                    canvas.drawText((int) progress[i] + "", (rectOvew.left + rectOvew.right) / 2,
                            rectOvew.top - 5, topTextPaint);
                } else {
            ...
            case MotionEvent.ACTION_UP:
                if (mType == 1) {
                    ...
                    for (int i = 0; i < circulars.size(); i++) {
                        ...
                        if (rect.right >= x && x >= rect.left && rect.top <= y && rect.bottom >=
                                y) {
                            if (mViewClick != null) {
    ...
    private int caculate(float value) {
        ...
        double startHeiht = totalHeight - (offset * 4.05);
